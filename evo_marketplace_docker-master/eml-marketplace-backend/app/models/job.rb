class Job < ApplicationRecord
  ...
  has_many :opted_freelancers, dependent: :destroy, source: :user
  has_many :rejected_freelancers, dependent: :destroy, source: :user
  has_many :job_cover_letters, dependent: :destroy
  accepts_nested_attributes_for :opted_freelancers, reject_if: :all_blank, allow_destroy: true
  accepts_nested_attributes_for :rejected_freelancers, reject_if: :all_blank, allow_destroy: true
  ...
  delegate :first_name, :full_name, :email, :avatar_url, :country, to: :user, allow_nil: true, prefix: true

  def first_freelancer_hired_on
    AcceptFreelancer.where(job_id: self.id).first.created_at if self.state == 'active' || self.state == 'completed'
  end

  def converted_starting_date
    self.starting_date.to_date
  end
  ...
  def job_cover_letter_count
    JobCoverLetter.where(job_id: self.id).count
  end
  ...
  def job_category
    @job_category = JobCategory.find_by(id: self.job_category_id).try(:name) if self.job_category_id.present?
    @job_category = self.category if self.job_category_id.nil?
  end

  def job_category_avatar_url
    @job_category_avatar_url = JobCategory.find_by(id: self.job_category_id).try(:avatar_url) if self.job_category_id.present?
    if self.job_category_id.nil?
      @home_category = HomeCategory.find_by(title: self.category)
      @job_category_avatar_url = @home_category.avatar_url if @home_category.present?
      @job_category_avatar_url = false if @home_category.nil?
    end
  end
