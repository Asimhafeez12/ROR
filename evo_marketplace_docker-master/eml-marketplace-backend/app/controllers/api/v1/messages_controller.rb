class Api::V1::MessagesController < ApplicationController
  ...
  def index
  	@messages = Chatroom.find(params[:chatroom_id]).chatroom_messages.where.not(user_id: current_user.id).mark_as_read! :all, for: current_user if params[:chatroom_id] != "undefined" && Chatroom.find(params[:chatroom_id]).chatroom_messages.unread_by(current_user).count > 0
