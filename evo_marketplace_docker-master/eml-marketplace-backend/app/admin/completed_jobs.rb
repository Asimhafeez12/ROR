ActiveAdmin.register Job, as: "Completed Jobs" do
  ...
  index do
    ...
    column :id
    column :title do |job|
      link_to job.title, admin_completed_job_path(job)
    end
    column :user do |job|
    ...
    actions defaults: false, dropdown: true do |job|
      item "Delete", admin_completed_job_path(job), method: :delete
    end
  end
  controller do
    def scoped_collection
      Job.where(state: "completed")
    end
  end
