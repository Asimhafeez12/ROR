ActiveAdmin.register User, as: "Unapproved Freelancers" do
  ...
  index do
    ...
    column :full_name do |user|
      link_to user.full_name, admin_unapproved_freelancer_path(user)
    end
    column :email
    column :is_approved
    column :approved_on
    ...
    actions defaults: false, dropdown: true do |user|
      item "Delete", admin_unapproved_freelancer_path(user), method: :delete
      item "#{user.is_approved && 'Disapprove' || 'Approve'} the freelancer", admin_unapproved_freelancer_path(user, user: { is_approved: !user.is_approved }), method: :patch
    end
  end
  ...
  controller do
    def scoped_collection
      User.where(is_approved: false).with_role(:freelancer).preload(:roles)
    end
    def update
      update! do |format|
        format.html { redirect_to [:admin, :unapproved_freelancers] }
      end
    end
  end
  form do |f|
