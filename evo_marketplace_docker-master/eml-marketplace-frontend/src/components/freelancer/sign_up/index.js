...
class FreelancerSignUp extends Component {
  ...
  beforeChange = (event) => {
	...
	const { updateSkillsetList } = this.props;
		...
		if(!err) {
		  ...
		  let data = {
			user: {
			  ...
			  skill_list: this.state.formList[1].currentObject.state.skillsList.join(", "),
	...
	render() {
		...
		return (
			<div className="freelancer-signup">
				...
				<button className="freelancer-signup-btn" onClick={this.beforeChange.bind(this)}>Next</button>
			</div>
		);
	}
}
...
function mapDispatchToProps(dispatch) {
  return {
	...
	save_skills: (data) => (dispatch(uActions.submitForm({user: data}))),
  }
}
