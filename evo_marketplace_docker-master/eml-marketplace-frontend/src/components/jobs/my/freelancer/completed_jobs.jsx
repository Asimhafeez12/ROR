...
class CompletedJobs extends Component {
	...
	componentWillMount() {
		this.props.fetchCompletedJobs(Defender.currentUser().id);
				...
				<div className="active-jobs">
					{completed_jobs && completed_jobs.length === 0 ?
						<div className="post-new-job-block">
							...
							<h2>You have no <strong>Close Job.</strong></h2>
						</div>
					:
						<React.Fragment>
							{completed_jobs.map( (job, index) =>
								<div className="active-jobs-holder" key={index}>
										...
										<div className="active-jobs-user-holder">
											<a href={`/profile/${job.user_id}`}>
												<div className="active-jobs-user-image">
						                            {!!job.user_avatar_url ?
						                                <React.Fragment>
						                                	<img src={job.user_avatar_url} alt="Wurker.ai - Connecting Top AI talent with Cool Companies" />
						                                </React.Fragment>
						                                :
									...
									<div className="active-jobs-right">
										<h2>Total Budget</h2>
										<span className="price-span">${(job.amount_paid_for_completed_milestones)/100}</span>
...
function mapDispatchToProps(dispatch) {
  return {
	fetchCompletedJobs: (user_id) => (dispatch(fcjActions.fetch(user_id))),
