...
import { Form, Icon, Rate, Button } from 'antd';
...
import ImageCroppingForm from './../static-pages/image-cropping';
...
class ClientProfile extends Component {
    ...
    state = {
	    ...
	    modal_image_cropping: false,
	    showImageCroppingForm: false,
	};
	...
	toggleImageCroppingForm(e) {
	    e.preventDefault();
	    this.setState({
	      showImageCroppingForm: !this.state.showImageCroppingForm,
	      modal_image_cropping: !this.state.modal_image_cropping,
	    });
	}
								...
								<div className="profile-content-top">
									<div className="profile-image-holder">
									  	{!!Defender.currentUser().avatar_url ?
									  		<img src={Defender.currentUser().avatar_url} alt="Wurker.ai - Connecting Top AI talent with Cool Companies" /> :
									  		<React.Fragment>
									...
									{ this.state.showImageCroppingForm ? <ImageCroppingForm cancelFunc={this.toggleImageCroppingForm.bind(this)} modal_image_cropping={this.state.modal_image_cropping}/> : ''}
									{Defender.currentUser().id === user.id ? 
									<a className="edit-btn" type="primary" onClick={this.toggleImageCroppingForm.bind(this)}><Icon style={{ color:'#7d7d7d', fontSize:'18px', fontWeight:'400'}} type="edit" /></a>
									: "" }
								...
								{completed_jobs && completed_jobs.map( (job, index) =>
									<div key={index} className="client2-jobs-holder">
										<div className="heading-block">
											<h3>{job.title}</h3>
											<ul>
												<li><strong>AI Freelancer : </strong>{job.accept_freelancers_list[0].user_full_name}</li>
												<li><time>{moment(job.created_at).format('MMMM YYYY')}</time></li>
											</ul>
											<span className="budget-text">${job.amount_paid_for_completed_milestones/100}</span>
										</div>
