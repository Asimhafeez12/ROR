  ...
  menu(){
    return (
      <Menu className="right-dropdown-menu">
        ...
        : "" }
        {Defender.currentUser()._r.includes("freelancer") && Defender.currentUser().is_approved === true  ?
        	<Menu.Item><a href="/freelancer/settings"><Icon type="setting" theme="outlined" /><span>Settings</span></a></Menu.Item>
        : "" }
        { Defender.currentUser()._r.includes("freelancer") && Defender.currentUser().is_approved === true ?
			<Menu.Item><a href="https://www.escrow-sandbox.com"><img className="available-earnings" src={wallet_icon} alt="Escrow | Wurker.ai - Connecting Top AI talent with Cool Companies" /><span>Available Earnings</span></a></Menu.Item>
		: "" }
		{ Defender.currentUser()._r.includes("client") || (Defender.currentUser()._r.includes("freelancer") && Defender.currentUser().is_approved === true) ?
			<Menu.Item>
				<a className="ant-dropdown-link" href="https://wurker.crisp.help/en/" rel="noopener noreferrer" target="_blank">
			...
			{chatrooms && chatrooms.length > 0 ?
				<Menu className="messages-dropdown-menu right-dropdown-menu">
					...
					{ chatrooms && chatrooms[0] !== undefined && chatrooms.map((chatroom, index) => (
						<Menu.Item className="messages-listing" key={index} onClick={ () => ( this.props.openChatroom({chatroom_id: chatroom.current_chatroom_id, user_id: chatroom.user.id, full_name: chatroom.user.full_name, job_id: chatroom.chatroom_job_id, job_title: chatroom.chatroom_job_title, job_state: chatroom.chatroom_job_state }) ) } onMouseDown={this.destoryWindow.bind()} >
							<a onClick={this.mark_messages_as_read.bind(this,chatroom.current_chatroom_id)}>
								<div className="list-content-holder">
									<div className="user-image-holder">
                                        {!!chatroom.user.avatar_url ?
                                            <React.Fragment>
                                                <img src={chatroom.user.avatar_url} alt="User Avatar | Wurker.ai - Connecting Top AI talent with Cool Companies"/>
                                            </React.Fragment>
                                            :
									  		<React.Fragment>
									  			{chatroom.user._r.includes("freelancer") ?
									  				<img className="available-earnings" src={FreelancerImage} alt="Freelancer | Wurker.ai - Connecting Top AI talent with Cool Companies" /> : ""
									  			}
									  			{chatroom.user._r.includes("client") ?
									  				<img className="available-earnings" src={ClientImage} alt="Client | Wurker.ai - Connecting Top AI talent with Cool Companies" /> : ""
									  			}
									  		</React.Fragment>
                                        }
									</div>
			...
			<div className="no-messages-dropdown">
				...
				<div className="no-messages-body">
					<div className="image-holder">
						<img src={message_icon} alt="Message | Wurker.ai - Connecting Top AI talent with Cool Companies" />
					</div>
					<div className="content">
						<h2>No Messages To Show</h2>
						{Defender.currentUser()._r.includes("client") ?
							<p>Hire any freelancer for your jobs to start conversation.</p>
						: 	<p>Conversation will be initiated with any client once you are invited for a job.</p>
						}
					</div>
				</div>
			</div>
...
{/*							{Defender.currentUser()._r.includes("client") ?
					...
					<div className="no-notification-dropdown">
						...
						<div className="no-notification-body">
							<div className="image-holder">
								<img src={notification_icon} alt="Notification | Wurker.ai - Connecting Top AI talent with Cool Companies" />
							</div>
							<div className="content">
		  ...
		  onReceived={this.onReceived.bind(this)} />
			<nav className="main-navigation">
			  ...
			  {Defender.currentUser()._r.includes("freelancer") ?
				<Menu.Item><a href="/freelancer"><Icon  style={{ fontSize:'18px' }} type="home" theme="outlined" /><span>Home</span></a></Menu.Item>
				:
				<Menu.Item><a href="/client"><Icon  style={{ fontSize:'18px' }} type="home" theme="outlined" /><span>Home</span></a></Menu.Item>
			   }
				{Defender.currentUser()._r.includes("client") || (Defender.currentUser()._r.includes("freelancer") && Defender.currentUser().is_approved === true) ?
			...
			<nav className="profile-menu">
			  <Menu style={{ overflow: 'visible' }}>
			  	{((Defender.currentUser()._r.includes("client") || (Defender.currentUser()._r.includes("freelancer") && Defender.currentUser().is_approved === true)) && window.location.href.split('/')[3] !== 'messages')  ?
					<Menu.Item>
				...
				{Defender.currentUser()._r.includes("client") || (Defender.currentUser()._r.includes("freelancer") && Defender.currentUser().is_approved === true) ?
					<Menu.Item>
					  <Dropdown placement="bottomLeft" style={{ top:'100', bottom:'100' }} overlay={this.notifications_list(notifications)} trigger={['click']}>
						<a className="ant-dropdown-link" onClick={this.mark_notification_as_read.bind(this,1)}>
				  ...
				  <Dropdown placement="bottomLeft" style={{ top:'100', bottom:'100' }} overlay={this.menu()} trigger={['click']}>
					<a className="ant-dropdown-link" href="">
					  <div className="my-image-holder">
                          ...
                          <React.Fragment>
                              <img src={Defender.currentUser().avatar_url} alt="User | Wurker.ai - Connecting Top AI talent with Cool Companies" />
                          </React.Fragment>
                          :
					  		<React.Fragment>
					  			{Defender.currentUser()._r.includes("freelancer") ?
					  ...
					  <span className="profile-username">{Defender.currentUser().first_name}</span>
					</a>
				  </Dropdown>
				</Menu.Item>
			  </Menu>
			  ...
			  <Dropdown placement="bottomRight" style={{ top:'100', bottom:'100' }} overlay={this.menu()} trigger={['click']}>
				<a className="ant-dropdown-link" href="">
				  <div className="my-image-holder">
					  	{!!Defender.currentUser().avatar_url ?
                          <React.Fragment>
                              <img src={Defender.currentUser().avatar_url} alt="Freelancer | Wurker.ai - Connecting Top AI talent with Cool Companies" />
                          </React.Fragment>
                          :
					  		<React.Fragment>
					  			{Defender.currentUser()._r.includes("freelancer") ?
